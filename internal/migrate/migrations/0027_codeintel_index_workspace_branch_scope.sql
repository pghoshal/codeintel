-- Scope CodeIntelIndex by Atom workspace and branch. Earlier split
-- executor tables already carried workspace/branch on subjobs and
-- manifests; this migration makes the persisted SCIP parent row match
-- that tenant/workspace/branch contract.
...
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'CodeIntelSymbol_languageIndexId_fkey') THEN
    ALTER TABLE "CodeIntelSymbol"
      DROP CONSTRAINT "CodeIntelSymbol_languageIndexId_fkey";
  END IF;
  IF EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'CodeIntelOccurrence_languageIndexId_fkey') THEN
    ALTER TABLE "CodeIntelOccurrence"
      DROP CONSTRAINT "CodeIntelOccurrence_languageIndexId_fkey";
  END IF;
  IF EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'CodeIntelRelationship_languageIndexId_fkey') THEN
    ALTER TABLE "CodeIntelRelationship"
      DROP CONSTRAINT "CodeIntelRelationship_languageIndexId_fkey";
  END IF;
