...
type Options struct {
	...
	// IgnoreDirs are runtime ignore directory names/globs supplied by the
	// caller. They are merged with .sourcegraph/ignore for every commit tree
	// without mutating the repository contents.
	IgnoreDirs []string
...
func newIgnoreMatcher(tree *object.Tree) (*ignore.Matcher, error) {
	return newIgnoreMatcherWithDirs(tree, nil)
}

func newIgnoreMatcherWithDirs(tree *object.Tree, ignoreDirs []string) (*ignore.Matcher, error) {
	...
	if err == nil {
		fileContent, err := ignoreFile.Contents()
		if err != nil {
			return nil, err
		}
	...
	for _, pattern := range runtimeIgnoreDirPatterns(ignoreDirs) {
	...
	return ignore.ParseIgnoreFile(strings.NewReader(content.String()))
}

func runtimeIgnoreDirPatterns(ignoreDirs []string) []string {
	var out []string
	seen := map[string]struct{}{}
	for _, raw := range ignoreDirs {
		...
		for _, item := range strings.Split(raw, ",") {
			...
			if strings.Contains(item, "\x00") || strings.HasPrefix(item, "../") || strings.Contains(item, "/../") || strings.HasSuffix(item, "/..") {
				continue
			}
			patterns := []string{
				item + "/**",
...
func prepareNormalBuild(options Options, repository *git.Repository) (repos map[fileKey]BlobLocation, branchVersions map[string]map[string]plumbing.Hash, err error) {
	...
	rw.ignoreDirs = append([]string(nil), options.IgnoreDirs...)
	for _, b := range branches {
		commit, err := getCommit(repository, options.BranchPrefix, b)
		if err != nil {
		...
		ig, err := newIgnoreMatcherWithDirs(tree, options.IgnoreDirs)
		if err != nil {
